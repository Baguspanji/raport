...
class Pengawas extends CI_Controller
{
    ...
    public function get_data($tipe = 1)
    {
        ...
        foreach ($list as $field) {
            ...
            $edit = '<a href="#" data-toggle="modal" data-target="#editPengawasModal"
				data-id="' . $field->id_laporan_penilaian . '" class="btn btn-warning btn-sm edit-modal"><i class="fas fa-edit"></i> Edit</a>';
			...
			$file = '<a href="#" data-toggle="modal" data-target="#showPengawasModal"
				data-id="' . $field->id_laporan_penilaian . '" class="btn btn-sm btn-info show-modal"><i class="fa fa-eye"></i> lihat File</a>';
    ...
    public function get_edit($tipe = 1, $id)
    {
        $role = $this->session->userdata('role');
        $list = $this->global->get_where('tb_laporan_penilaian', ['tipe_laporan' => $tipe, 'id_laporan_penilaian' => $id]);
        if ($role == 'pengawas') {
            $list = $this->global->get_where('tb_laporan_penilaian', ['tipe_laporan' => $tipe, 'id_laporan_penilaian' => $id], true);
        }
